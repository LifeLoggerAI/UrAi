...
function box(name, size, mat, position, rotation = [0, 0, 0], scale = [1, 1, 1]) {
...
function limb(name, radius, length, mat, position, rotation = [0, 0, 0]) {
  ...
  group.name = name;
  group.add(
    cylinder(`${name}-body`, radius * 0.9, radius, length, mat, [0, 0, 0], [0, 0, 0], 16),
    sphere(`${name}-cap-a`, radius, mat, [0, length / 2, 0], [1, 0.9, 1], 16),
    sphere(`${name}-cap-b`, radius * 0.92, mat, [0, -length / 2, 0], [1, 0.9, 1], 16),
  );
...
function createHuman({ name = 'urai-human', skin = M.skin, shirt = M.clothBlue, hair = M.hair, heightScale = 1, pose = 0 } = {}) {
  ...
  root.name = `${name}-root`;
  root.userData = { assetType: 'human', units: 'meters', upAxis: 'Y', nominalHeightMeters: 1.82 * heightScale, cameraAspect: ASPECT, status: 'runtime-human-base-v1', finalTarget: 'rigged-scan-grade-human-with-facial-blendshapes' };

  const hips = new THREE.Group(); hips.name = `${name}-hips`; hips.position.y = 0.93; root.add(hips);
  ...
  const head = new THREE.Group(); head.name = `${name}-head-rig`; head.position.set(0, 0.49, 0); chest.add(head);
  head.add(sphere(`${name}-head`, 1, skin, [0, 0, 0], [0.112, 0.145, 0.105], 28));
  head.add(sphere(`${name}-ear-left`, 1, skin, [-0.113, 0, 0], [0.016, 0.032, 0.018], 14));
  head.add(sphere(`${name}-ear-right`, 1, skin, [0.113, 0, 0], [0.016, 0.032, 0.018], 14));
  head.add(mesh(new THREE.ConeGeometry(0.014, 0.043, 12), skin, `${name}-nose`, [0, -0.005, 0.105], [Math.PI / 2, 0, 0]));
  for (const [side, x] of [['left', -0.04], ['right', 0.04]]) {
    head.add(sphere(`${name}-eye-${side}`, 1, M.eyeWhite, [x, 0.025, 0.101], [0.019, 0.01, 0.006], 14));
    head.add(sphere(`${name}-iris-${side}`, 1, M.eye, [x, 0.025, 0.108], [0.006, 0.006, 0.004], 12));
  }
  head.add(sphere(`${name}-mouth`, 1, M.lip, [0, -0.055, 0.1], [0.034, 0.006, 0.006], 12));
  head.add(sphere(`${name}-hair-cap`, 1, hair, [0, 0.062, -0.027], [0.118, 0.105, 0.11], 22));
  head.add(sphere(`${name}-hair-left`, 1, hair, [-0.098, 0, -0.028], [0.027, 0.075, 0.035], 14));
  head.add(sphere(`${name}-hair-right`, 1, hair, [0.098, 0, -0.028], [0.027, 0.075, 0.035], 14));
...
function createHomeWorld() {
  ...
  root.add(cylinder('home-pond', 3, 3, 0.025, M.water, [2.8, -0.22, -1.5], [0, 0, 0], 64));
  for (let i = 0; i < 17; i += 1) root.add(box(`path-stone-${i}`, [0.56 + (i % 3) * 0.07, 0.07, 0.42], M.stoneWarm, [-1.4 + i * 0.09, 0.14, 7.5 - i * 0.7], [0, (i % 3 - 1) * 0.035, 0]));
  [[-8,-3,.5],[-6,5,.38],[8,3,.54],[5,-6,.45],[-2,-7,.42],[9,-5,.35]].forEach(([x,z,r],i)=>root.add(sphere(`rock-${i}`,1,M.rock,[x,r*.55,z],[r*1.35,r*.75,r],12)));
  [[-7,-2,1],[-10,5,1.2],[9,4,1.15],[7,-7,.95],[-4,-9,.9],[11,-3,.8]].forEach(([x,z,s],i)=>{
    ...
    root.add(sphere(`tree-crown-${i}-left`,1,M.leaf,[x-.62*s,3*s,z+.1*s],[.68*s,.55*s,.7*s],12));
...
function createCouncilChamber(withPeople = true) {
  ...
  seats.forEach(([x,z],i)=>{ root.add(box(`chair-seat-${i}`,[.68,.16,.62],M.clothGray,[x,.42,z])); root.add(box(`chair-back-${i}`,[.66,.82,.15],M.clothGray,[x,.86,z+.25])); });
  if (withPeople) {
    const people=[['guide',M.skin,M.clothBlue],['mirror',M.skinDark,M.clothGray],['guardian',M.skinDark,M.clothBlue],['archivist',M.skinLight,M.clothWarm],['builder',M.skin,M.clothWarm],['trickster',M.skin,M.clothGray]];
...
function createShadowWorld() {
  const root = new THREE.Group(); root.name = 'urai-shadow-world'; root.userData = { realm:'Shadow',units:'meters',cameraAspect:ASPECT,status:'runtime-world-v1',governance:'visual-substrate-only' };
  root.add(box('shadow-floor',[10.5,.18,14],M.stoneDark,[0,-.09,-1.5])); root.add(box('shadow-wall-left',[.3,5,14],M.stoneDark,[-5.2,2.5,-1.5])); root.add(box('shadow-wall-right',[.3,5,14],M.stoneDark,[5.2,2.5,-1.5])); root.add(box('shadow-back',[10.5,5,.3],M.stone,[0,2.5,-8.4]));
  for(let i=0;i<18;i+=1) root.add(box(`shadow-path-${i}`,[1.05,.055,.48],M.stoneWarm,[0,.03,5.2-i*.65],[0,(i%3-1)*.025,0]));
  root.add(cylinder('shadow-basin',1.65,1.65,.22,M.stone,[0,.1,-5.2],[0,0,0],56)); root.add(cylinder('shadow-basin-water',1.35,1.35,.025,M.water,[0,.23,-5.2],[0,0,0],56));
...
function createMirrorWorld() {
  const root = new THREE.Group(); root.name='urai-mirror-world'; root.userData={realm:'Mirror',units:'meters',cameraAspect:ASPECT,status:'runtime-world-v1',reflectionPolicy:'physical-space-under-governed-reflection-ui'};
  root.add(box('mirror-floor',[12,.18,15],M.stoneWarm,[0,-.09,-2])); root.add(box('mirror-back-wall',[12,5.2,.3],M.stone,[0,2.6,-9.3])); root.add(box('mirror-wall-left',[.3,5.2,15],M.stone,[-6,2.6,-2])); root.add(box('mirror-wall-right',[.3,5.2,15],M.stone,[6,2.6,-2]));
  root.add(box('mirror-frame-top',[4.25,.24,.28],M.bronze,[0,4.15,-8.95])); root.add(box('mirror-frame-bottom',[4.25,.24,.28],M.bronze,[0,.55,-8.95])); root.add(box('mirror-frame-left',[.24,3.85,.28],M.bronze,[-2.05,2.35,-8.95])); root.add(box('mirror-frame-right',[.24,3.85,.28],M.bronze,[2.05,2.35,-8.95])); root.add(box('mirror-surface',[3.8,3.45,.06],M.mirror,[0,2.35,-9]));
  for(const x of [-4.65,4.65]){root.add(box(`mirror-side-panel-${x}`,[1.55,2.75,.08],M.glass,[x,1.9,-6.2],[0,x<0?.12:-.12,0])); root.add(box(`mirror-bench-${x}`,[1.8,.42,.72],M.wood,[x,.25,-3.8]));}
  for(let i=0;i<10;i+=1) root.add(box(`mirror-path-${i}`,[1.35,.05,.58],M.stoneWarm,[Math.sin(i*.8)*.08,.03,3.5-i*.9],[0,(i%3-1)*.02,0])); return root;
}

function createLegacyWorld() {
  const root=new THREE.Group(); root.name='urai-legacy-world'; root.userData={realm:'Legacy',units:'meters',cameraAspect:ASPECT,status:'runtime-world-v1',governance:'user-approved-memory-archive-only'};
  root.add(box('legacy-floor',[13,.18,17],M.stoneWarm,[0,-.09,-2.5])); root.add(box('legacy-back-wall',[13,5.2,.3],M.stoneWarm,[0,2.6,-10.8])); root.add(box('legacy-wall-left',[.3,5.2,17],M.stone,[-6.5,2.6,-2.5])); root.add(box('legacy-wall-right',[.3,5.2,17],M.stone,[6.5,2.6,-2.5]));
  for(const side of [-1,1]) for(let row=0;row<4;row+=1){ const z=2-row*3.2,x=side*5.45; root.add(box(`legacy-shelf-${side}-${row}`,[1.45,2.25,.48],M.woodDark,[x,1.25,z])); for(let shelf=0;shelf<4;shelf+=1) root.add(box(`legacy-shelf-board-${side}-${row}-${shelf}`,[1.55,.08,.58],M.wood,[x,.25+shelf*.62,z])); for(let book=0;book<7;book+=1) root.add(box(`legacy-book-${side}-${row}-${book}`,[.11,.36,.28],book%2?M.paper:M.clothWarm,[x+(book-3)*.17,.52,z-.33])); }
  root.add(box('legacy-reading-table',[3.4,.14,1.25],M.wood,[0,.78,-1.8])); root.add(box('legacy-table-base',[.7,.75,.65],M.woodDark,[0,.38,-1.8]));
  for(let i=0;i<5;i+=1){const x=-4+i*2; root.add(box(`legacy-frame-${i}`,[1.3,1.65,.08],M.bronze,[x,2.55,-10.55])); root.add(box(`legacy-memory-surface-${i}`,[1.1,1.4,.04],M.glass,[x,2.55,-10.49]));}
  for(const x of [-4.7,4.7]) root.add(cylinder(`legacy-lamp-${x}`,.12,.14,.42,M.lamp,[x,.22,-4.8],[0,0,0],16)); return root;
}

async function exportGLB(filename, sceneRoot) {
  const scene = new THREE.Scene(); scene.name = filename.replace(/\.glb$/, ''); scene.userData = { ...sceneRoot.userData, generatedBy:'scripts/assets/build-real-3d-assets.mjs', generatedAt:new Date().toISOString() }; scene.add(sceneRoot); scene.updateMatrixWorld(true);
  const exporter = new GLTFExporter();
  const result = await exporter.parseAsync(scene, { binary:true, onlyVisible:true, trs:true });
  if (!(result instanceof ArrayBuffer)) throw new Error(`Expected binary GLB ArrayBuffer for ${filename}`);
  const bytes = Buffer.from(result); const path = join(OUTPUT_DIR, filename); await mkdir(dirname(path), { recursive:true }); await writeFile(path, bytes);
  return { file:filename, bytes:bytes.length, sha256:createHash('sha256').update(bytes).digest('hex'), scene:scene.name, cameraAspect:ASPECT, units:'meters' };
}
