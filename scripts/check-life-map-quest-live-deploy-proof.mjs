...
const liveDoc = read("launch-proof/life-map-quest-interaction/LIFE_MAP_QUEST_LIVE_DEPLOY_VERIFICATION.md");
const manifest = readJson("launch-proof/life-map-quest-interaction/life-map-quest-proof-manifest.json");
...
assertCheck(
  ...
  liveDoc.includes("LIFE MAP QUEST DEPLOY-CONFIGURED") &&
    liveDoc.includes("LIFE MAP QUEST LIVE-SMOKE-PASSED") &&
    liveDoc.includes("LIFE MAP QUEST LIVE-QUEST-VERIFIED"),
  "LIFE_MAP_QUEST_LIVE_DEPLOY_VERIFICATION.md must preserve the deploy-configured/live-smoke/live-Quest status ladder.",
);
...
assertCheck(
  ...
  liveDoc.includes("deployed desktop `/life-map`") &&
    liveDoc.includes("Quest Browser can reach deployed `/life-map`") &&
    liveDoc.includes("VR session starts from deployed `/life-map`"),
...
assertCheck(
  ...
  Array.isArray(manifest?.blocked_claims_until_verified) &&
    manifest.blocked_claims_until_verified.includes("Quest ready") &&
    manifest.blocked_claims_until_verified.includes("VR ready") &&
    manifest.blocked_claims_until_verified.includes("production XR"),
  "Life Map Quest manifest must keep unverified Quest/VR/XR claims blocked.",
);
...
if (failed.length) {
  console.error(`\nLife Map Quest live deploy proof check failed: ${failed.length} failing check(s).`);
  process.exit(1);
}

console.log("\nLife Map Quest live deploy proof check passed.");
