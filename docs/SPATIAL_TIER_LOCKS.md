...
Status: Tier-2 preparation layer. This document does not unlock public Tier-2 behavior by itself.
...
Tier-2 Personal Spatial features must render a silent Tier-1 fallback until all of these are true:
...
This layer is safe to build during Tier-1 lock because it does not change the locked routes, home scene, public routing, Firebase rules, or live data capture.
...
1. Add Firestore rule coverage for `features`, `entitlements`, `consents`, and `auditLogs`.
2. Add server-side `evaluateSpatialTierLock(featureId)` backed by authenticated user context.
3. Add a `useSpatialTierLock(featureId)` hook that calls the server evaluator.
4. Add Playwright coverage proving Tier-1 fallback and Tier-2 unlock.
5. Wire Tier-2 UI only behind flags after Tier-1 is sealed.
