...
export const createDarRequest = onCall(async (request: CallableRequest) => {
  ...
  if (!partnerId || !packageId || !apiKey) {
    throw new HttpsError(
      'invalid-argument',
      'Missing partnerId, packageId, or apiKey.'
    );
  }
  ...
  if (!packageDoc.exists) {
    throw new HttpsError(
      'not-found',
      'The specified data package does not exist.'
    );
  }
...
export const approveDarRequest = onCall(async (request: CallableRequest) => {
  if (!request.auth?.token.admin) {
    throw new HttpsError(
      'permission-denied',
      'Must be an admin to approve requests.'
    );
  }
...
export const rejectDarRequest = onCall(async (request: CallableRequest) => {
  if (!request.auth?.token.admin) {
    throw new HttpsError(
      'permission-denied',
      'Must be an admin to reject requests.'
    );
  }
  ...
  if (!requestId || !notes) {
    throw new HttpsError(
      'invalid-argument',
      'Request ID and rejection notes are required.'
    );
  }
