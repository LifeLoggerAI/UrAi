...
const COLLECTION_MAPPINGS: CollectionMapping[] = [
  ...
  },
  {
    ...
    fields: [
      ...
      {output: "nocturnalDrift", fields: ["sleepDebt", "nocturnalDrift", "nightLoad"]},
    ],
  },
  {
  ...
  },
  {
    ...
    timestampFields: ["createdAt", "timestamp", "date"],
    fields: [
      {output: "socialAbsence", fields: ["socialAbsence", "lastHeardDeltaScore", "absenceWeight"]},
      {output: "connectionPull", fields: ["connectionPull", "closenessScore", "relationshipWarmth"]},
      {output: "conflictResidue", fields: ["conflictResidue", "conflictScore", "tensionScore"]},
    ],
  },
  {
    collection: "events",
    ...
    fields: [
      ...
      {output: "sighLikelihood", fields: ["sighLikelihood", "sighScore"]},
...
function getMappedValue(data: FirebaseFirestore.DocumentData, mapping: FieldMapping): number | null {
  ...
  const averaged = avg(values);
  return mapping.transform ? clamp01(mapping.transform(averaged)) : averaged;
}
...
async function queryCollectionWindow(
  ...
  for (const timestampField of mapping.timestampFields) {
    try {
      ...
      if (!snapshot.empty) {
        return snapshot.docs.map((doc) => ({id: doc.id, data: doc.data(), timestampField}));
      }
    } catch (error) {
  ...
  try {
    ...
    return snapshot.docs.map((doc) => ({id: doc.id, data: doc.data(), timestampField: null}));
  } catch (error) {
...
export async function buildAncientSignalsFromPassiveRollups(
  ...
  for (const mapping of COLLECTION_MAPPINGS) {
    const docs = await queryCollectionWindow(
      firestore,
      options.ownerUid,
      mapping,
      options.startAt,
    ...
    for (const doc of docs) {
      for (const fieldMapping of mapping.fields) {
        const value = getMappedValue(doc.data, fieldMapping);
  ...
  const input = Object.fromEntries(
    Object.entries(buckets).map(([key, values]) => [key, avg(values)])
  );

  const durationMinutes = Math.max(
    1,
    Math.round((new Date(options.endAt).getTime() - new Date(options.startAt).getTime()) / 60000)
  );
