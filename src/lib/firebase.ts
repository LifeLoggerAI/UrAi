...
try {
  ...
  console.warn(
    'Firestore initialization with persistence failed, falling back to in-memory cache. Error:',
    e
  );
  db = getFirestore(app);
}
...
if (devMode) {
  ...
  try {
    ...
    console.warn(
      'Could not connect to emulators, assuming they are not running. Error: ',
      e
    );
  }
}
