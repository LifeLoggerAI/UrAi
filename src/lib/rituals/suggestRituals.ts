...
function ritualFromTemplate(template: RitualTemplate, sourceType: UraiRitual["sourceType"] = "system", sourceId?: string, sourceLayerIds: PassportDataLayerId[] = ["system"]): UraiRitual {
  return { id: `ritual-${template.id}-${sourceId ?? "system"}-${Date.now()}`, title: template.title, subtitle: template.subtitle, type: template.type, intensity: template.intensity, status: "suggested", summary: template.summary, steps: template.steps, sourceType, sourceId, sourceLayerIds, createdAt: now(), exportAllowed: false, addToLegacyAllowed: false };
}
...
export function suggestRituals(context: RitualSuggestionContext = {}): UraiRitual[] {
  ...
  if (context.selectedShadowReflection?.id && context.selectedShadowReflection.visibility !== "locked") {
    const layers: PassportDataLayerId[] = context.selectedShadowReflection.sourceLayerIds?.length ? context.selectedShadowReflection.sourceLayerIds : ["shadow"];
  ...
  if (context.selectedGroundElement?.id) {
    const layer: PassportDataLayerId = context.selectedGroundElement.sourceLayerId ?? "system";
  ...
  if (context.selectedLifeMapStar?.id) {
    const layer: PassportDataLayerId = context.selectedLifeMapStar.sourceLayerId ?? "system";
  ...
  if (context.selectedMirrorReflection?.id) {
    const layers: PassportDataLayerId[] = context.selectedMirrorReflection.sourceLayerIds?.length ? context.selectedMirrorReflection.sourceLayerIds : ["system"];
  ...
  if (context.selectedLegacyItem?.id) {
    const layers: PassportDataLayerId[] = context.selectedLegacyItem.sourceLayerIds?.length ? context.selectedLegacyItem.sourceLayerIds : ["legacy"];
  ...
  return suggestions.filter((item) => allowedTemplate(context, item.template, item.layers)).slice(0, 3).map((item) => ritualFromTemplate(item.template, item.sourceType, item.sourceId, item.layers));
}
