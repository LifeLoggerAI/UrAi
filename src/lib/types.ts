...
export const EnrichVoiceEventOutputSchema = z.object({
  ...
  sentimentScore: z
    .number()
    .describe('A score from -1 (negative) to 1 (positive) representing the overall tone.'),
  toneShift: z.number().describe('A score representing the change in tone within the text.'),
  voiceArchetype: z
    .string()
    .describe("The social archetype displayed (e.g., 'Mentor', 'Friend', 'Reporter')."),
  people: z.array(z.string()).describe('A list of names of people mentioned in the transcript.'),
  tasks: z
    .array(z.string())
    .describe('A list of actionable tasks or to-do items mentioned in the transcript.'),
});
...
export const AnalyzeDreamOutputSchema = z.object({
  ...
  symbols: z
    .array(z.string())
    .describe("A list of key symbols and their potential meanings within the dream's context."),
  sentimentScore: z
    .number()
    .describe("A score from -1 (very negative) to 1 (very positive) for the dream's overall tone."),
});
...
export const AnalyzeCameraImageOutputSchema = z.object({
  ...
  backgroundMoodTags: z
    .array(z.string())
    .describe('An array of tags describing the emotional mood of the background.'),
  contextualSymbolMatches: z.array(z.string()).describe('An array of recognized symbolic fusions.'),
  linkedArchetype: z
    .string()
    .describe('The primary Jungian archetype that this image seems to represent.'),
});
...
export const SuggestRitualInputSchema = z.object({
  ...
  context: z
    .string()
    .describe("Brief context about the user's current state, like overall mood or recent themes."),
});
...
export const SuggestRitualOutputSchema = z.object({
  ...
  description: z
    .string()
    .describe('A one or two-sentence description of the ritual, framed as a gentle suggestion.'),
...
export const AvatarIdentityProgressSchema = z.object({
  ...
  featureStage: z.enum(['blank', 'limbs', 'tone', 'faceShape', 'clothing', 'artifact', 'locked']),
