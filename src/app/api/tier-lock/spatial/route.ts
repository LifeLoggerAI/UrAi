...
async function evaluateSpatialTierLock(req: Request, featureId: TierLockFeatureId) {
...
export async function POST(req: Request) {
  try {
    const body = (await req.json()) as SpatialTierLockRequest;
    if (!isTierLockFeatureId(body.featureId)) {
      return NextResponse.json({ error: "A valid Tier-2 featureId is required." }, { status: 400 });
    }

    return evaluateSpatialTierLock(req, body.featureId);
  } catch {
    return NextResponse.json({ error: "Unable to evaluate spatial tier lock." }, { status: 400 });
  }
}
