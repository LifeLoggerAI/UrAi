...
export async function summarizeWeekAction(
  userId: string
): Promise<{ summary: string | null; audioDataUri: string | null; error: string | null }> {
  if (!userId) {
  ...
  try {
    ...
    const allTranscripts = querySnapshot.docs.map(doc => doc.data().text).join('\n\n---\n\n');
...
export async function analyzeAndLogCameraFrameAction(input: {
  ...
  try {
    ...
    const primaryEmotion = Object.keys(analysis.emotionInference).reduce(
      (a, b) => (analysis.emotionInference[a] > analysis.emotionInference[b] ? a : b),
      'neutral'
    );
