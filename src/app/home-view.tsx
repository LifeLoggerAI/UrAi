...
function SkyLayer({ mode, pointer, transitioning, reducedMotion }: { mode: HomeMode; pointer: PointerState; transitioning: boolean; reducedMotion: boolean }) {
  ...
  return (
    <div className="pointer-events-none absolute inset-0 overflow-hidden">
      ...
      <div className={`absolute left-1/2 top-[26%] h-[1px] w-[58rem] -translate-x-1/2 bg-gradient-to-r from-transparent via-cyan-100/20 to-transparent transition-opacity duration-500 ${mode === "memory" || transitioning ? "opacity-90" : "opacity-45"}`} />
    </div>
  );
}

function GroundLayer({ mode, pointer, reducedMotion }: { mode: HomeMode; pointer: PointerState; reducedMotion: boolean }) {
  const transform = reducedMotion ? undefined : `translate3d(${pointer.x * 7}px, ${pointer.y * 4}px, 0)`;
  return (
    <div className="pointer-events-none absolute inset-x-0 bottom-0 h-[45%] overflow-hidden" style={{ transform }}>
      ...
      <div className={`absolute left-1/2 bottom-[11%] h-32 w-[44rem] -translate-x-1/2 rounded-[50%] blur-2xl ${mode === "bloom" ? "bg-emerald-200/28" : "bg-cyan-200/12"} ${reducedMotion ? "" : "animate-[uraiGroundBloom_6s_ease-in-out_infinite]"}`} />
  ...
  return (
    <>
      ...
      >
        ...
        <GroundLayer mode={mode} pointer={pointer} reducedMotion={reducedMotion} />
        ...
        <section className="relative z-10 mx-auto flex min-h-screen max-w-6xl flex-col items-center justify-center px-6 py-12 text-center md:py-16">
          ...
          <div className="mt-8 flex flex-wrap justify-center gap-2">
            {(["quiet", "listening", "memory", "bloom"] as HomeMode[]).map((item) => (
              <button key={item} type="button" onClick={() => setMode(item)} className={`rounded-full border px-3 py-2 text-[11px] font-semibold uppercase tracking-[0.2em] transition focus:outline-none focus:ring-2 focus:ring-cyan-100/60 ${mode === item ? "border-cyan-100/40 bg-cyan-100/15 text-cyan-50" : "border-white/10 bg-white/[0.05] text-white/45 hover:text-white/80"}`}>{item}</button>
            ))}
          </div>

          <div className="mt-9"><OrbLayer mode={mode} pointer={pointer} reducedMotion={reducedMotion} onOpen={() => { setMode("listening"); setOrbOpen(true); }} /></div>
          ...
          <div className="mt-8 flex flex-wrap justify-center gap-3">
            ...
            <button type="button" onClick={() => { setMode("listening"); setOrbOpen(true); }} className="rounded-full border border-white/14 bg-white/[0.08] px-6 py-3 text-sm font-semibold text-white backdrop-blur-xl transition hover:bg-white/[0.13] focus:outline-none focus:ring-2 focus:ring-cyan-100/60">Talk to URAI</button>
            <Link href="/settings/privacy" className="rounded-full border border-emerald-200/20 bg-emerald-300/[0.08] px-6 py-3 text-sm font-semibold text-emerald-50 backdrop-blur-xl transition hover:bg-emerald-300/[0.13] focus:outline-none focus:ring-2 focus:ring-emerald-100/60">Privacy Controls</Link>
          </div>
