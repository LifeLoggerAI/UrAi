 ...
 */
export const createDefaultProfile = functions.auth.user().onCreate(
  async (user) => {
    ...
    try {
      const newUserDoc = {
        ...
        avatarUrl: photoURL ||
          `https://placehold.co/128x128.png?text=${(displayName ||
            email ||
      ...
      functions.logger.error(
        `Error creating profile for user ${uid}:`,
        error
      );
    }
  }
...
export const onUserDelete = functions.auth.user().onDelete(async (user) => {
  ...
  try {
    ...
    functions.logger.info(
      `Successfully deleted user profile for ${uid}. Further cleanup is ` +
      "required for a production app."
    );
  } catch (error) {
    functions.logger.error(
      `Error deleting user profile for ${uid}:`,
      error
    );
  }
});
