 ...
 */
export const ingestTimelineEvent = functions.https.onCall(
  async (data, context) => {
...
export const detectShadowEpisode = functions.firestore
  ...
  .onWrite(async (change, context) => {
    functions.logger.info(
      `Checking for shadow episode for user ${context.params.uid}.`
    );
...
export const evaluateLegacyProgress = functions.firestore
  ...
  .onUpdate(async (change, context) => {
    functions.logger.info(
      `Evaluating legacy progress for user ${context.params.uid}.`
    );
