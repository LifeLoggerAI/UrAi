...
export function LegacyItemDetail({ item, onClose, onApprove, onSeal, onUnseal, onRemove, onOpenPassport, onReflect, onOpenLifeMap, onOpenGround }: LegacyItemDetailProps) {
  ...
  const createRitual = () => {
    if (locked || sealed) return;
    const [ritual] = rituals.suggestForContext({ selectedLegacyItem: { id: item.id, type: item.type, sourceLayerIds: item.sourceLayerIds }, passportProfile: { ritualsEnabled: true, legacyEnabled: true, enabledLayers: { legacy: true, system: true } } });
    if (ritual) rituals.startRitual(ritual.id);
  };
  const exportScroll = () => {
    if (locked || sealed) return;
    exports.openExport({ id: `export-legacy-${item.id}`, type: item.type === "chapter" ? "chapter_scroll" : "legacy_scroll", title: item.title, subtitle: item.subtitle, summary: item.summary, format: "png", privacyLevel: "summary_only", sourceType: "legacy", sourceIds: [item.id], sourceLayerIds: item.sourceLayerIds, userApproved: true });
  };
  return (
    <aside className="pointer-events-auto absolute inset-x-4 bottom-4 z-30 rounded-3xl border border-white/10 bg-black/44 p-4 text-white/82 shadow-2xl backdrop-blur-xl md:left-auto md:right-6 md:top-24 md:w-[390px] md:bottom-auto">
      <div className="flex items-start justify-between gap-3"><div><p className="text-xs uppercase tracking-[0.24em] text-white/45">{item.type.replace(/_/g, " ")}</p><h3 className="mt-1 text-lg font-medium text-white">{item.title}</h3>{item.subtitle ? <p className="mt-1 text-sm text-white/62">{item.subtitle}</p> : null}</div><button type="button" onClick={onClose} aria-label="Close Legacy item" className="rounded-full bg-white/10 px-2 py-1 text-white/70">×</button></div>
      ...
      <dl className="mt-4 grid gap-1 text-xs text-white/45"><div className="flex justify-between gap-3"><dt>Date</dt><dd>{formatDate(item.sourceCreatedAt ?? item.createdAt)}</dd></div><div className="flex justify-between gap-3"><dt>Status</dt><dd>{sealed ? "sealed" : item.visibility}</dd></div><div className="flex justify-between gap-3"><dt>Approval</dt><dd>{item.userApproved ? "approved" : "not approved"}</dd></div><div className="flex justify-between gap-3"><dt>Export</dt><dd>{item.exportAllowed ? "allowed" : "review first"}</dd></div></dl>
      <div className="mt-4 flex flex-wrap gap-2">
        ...
        {!locked && !sealed ? <button type="button" onClick={createRitual} className="rounded-full bg-white/10 px-3 py-2 text-xs text-white/84">Create Legacy ritual</button> : null}
        {!locked && !sealed ? <button type="button" onClick={exportScroll} className="rounded-full bg-white/[0.08] px-3 py-2 text-xs text-white/62">Export scroll</button> : null}
