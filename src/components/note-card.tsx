...
import { BrainCircuit, TrendingUp, Zap, MicVocal } from "lucide-react";
...
export function NoteCard({ item }: { item: VoiceEvent }) {
  ...
  const sentiment = item.sentimentScore > 0.1 ? 'positive' : item.sentimentScore < -0.1 ? 'negative' : 'neutral';
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4 w-full">
                <div className="space-y-3">
                    <div className="flex flex-wrap items-center gap-2 text-sm">
                        ...
                        <Badge variant="outline" className="capitalize">{item.emotion}</Badge>
                    </div>
                     <div className="flex flex-wrap items-center gap-2 text-sm">
                        <MicVocal className="h-4 w-4 text-muted-foreground flex-shrink-0" />
                        <Badge variant="secondary" className="capitalize">{item.speakerLabel}</Badge>
                    </div>
                </div>
                 <div className="space-y-3">
                    <div className="flex items-center gap-3 text-sm">
                        ...
                        <Progress value={sentimentScoreNormalized} className="w-full" indicatorClassName={getProgressColor(item.sentimentScore)} />
                        <span className="text-muted-foreground text-xs w-10 text-right">{item.sentimentScore.toFixed(2)}</span>
                    </div>
                    <div className="flex items-center gap-3 text-sm">
                        <Zap className="h-4 w-4 text-muted-foreground" />
                        <span className="font-medium w-32 shrink-0">Tone Shift</span>
                        <Progress value={item.toneShift * 100} className="w-full" />
                        <span className="text-muted-foreground text-xs w-10 text-right">{item.toneShift.toFixed(2)}</span>
                    </div>
                </div>
            </div>
        </CardFooter>
