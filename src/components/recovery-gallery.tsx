    ...
    return (
      <div className="text-center text-muted-foreground py-16 px-4 bg-muted/50 border rounded-lg animate-fadeIn">
        ...
        <h3 className="mt-4 text-lg font-medium text-foreground">
          No Weekly Scrolls Yet
        </h3>
        <p className="mt-1 text-sm">
          Your first automated weekly story will be generated on Sunday. Keep
          logging memories!
        </p>
      </div>
    );
      ...
      {scrolls.map(scroll => (
        <Card key={scroll.id} className="animate-fadeIn">
            ...
            <div className="flex justify-between items-start">
              <div>
                <CardTitle>
                  {scroll.title
                    ? scroll.title
                    : `Weekly Scroll: ${new Date(scroll.weekStart!).toLocaleDateString()}`}
                </CardTitle>
                <CardDescription>
                  {scroll.title
          ...
          <CardContent className="space-y-4">
            <p className="whitespace-pre-wrap text-sm text-foreground/80 italic">
              &ldquo;{scroll.narrationScript}&rdquo;
            </p>

            {scroll.segments && scroll.segments.length > 0 && (
              <div>
                <h4 className="font-semibold text-sm mb-2 flex items-center gap-2">
                  <Milestone className="text-primary h-4 w-4" /> Story Segments
                </h4>
            ...
            {scroll.highlights && scroll.highlights.length > 0 && (
              <div>
                <h4 className="font-semibold text-sm mb-2 flex items-center gap-2">
                  <Lightbulb className="text-primary h-4 w-4" /> Highlights
                </h4>
                <ul className="list-none space-y-2">
                  {scroll.highlights.map((highlight, index) => (
                    <li
                      key={index}
                      className="flex items-center gap-3 text-sm text-foreground/80 p-2 bg-muted rounded-md"
                    >
                      {highlight.type === 'recovery' ? (
          ...
          <CardFooter className="flex-wrap gap-x-4 gap-y-2 justify-between items-center">
            <div className="flex gap-2">
              <Button
                variant="outline"
                size="sm"
                onClick={() => handleExport(scroll.exportLinks.audio)}
              >
                <Mic className="mr-2 h-4 w-4" /> Export Audio
              </Button>
              <Button
                variant="outline"
                size="sm"
                onClick={() => handleExport(scroll.exportLinks.image)}
              >
