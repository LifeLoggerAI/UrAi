...
export function GroundElementDetail({ element, onClose, onOpenPassport, onReflect }: GroundElementDetailProps) {
  ...
  const summary = element.summary ?? element.subtitle ?? "A Ground symbol carried forward as a summary.";
  const layer = element.sourceLayerId ?? "system";
  const addToLegacy = () => {
    if (locked) return;
    legacy.addItemToLegacy(legacyCandidateFromSummary({ id: `legacy-ground-${element.id}`, type: element.type === "recoveryBloom" ? "recovery" : "ground_bloom", title: element.title, summary, sourceLayerIds: [layer], tone: "grounded", linkedGroundElementId: element.id }));
    legacy.openLegacy();
  };
  const createRitual = () => {
    if (locked) return;
    const [ritual] = rituals.suggestForContext({ selectedGroundElement: { id: element.id, type: element.type, sourceLayerId: layer } });
    if (ritual) rituals.startRitual(ritual.id);
  };
  const exportCard = () => {
    if (locked) return;
    exports.openExport({ id: `export-ground-${element.id}`, type: element.type === "ritualSeed" ? "ritual_card" : "ground_bloom_card", title: element.title, subtitle: element.subtitle, summary, format: "png", privacyLevel: "summary_only", sourceType: "ground", sourceIds: [element.id], sourceLayerIds: [layer], userApproved: true });
  };
  return (
    <aside className="pointer-events-auto absolute inset-x-4 bottom-4 z-30 rounded-3xl border border-white/10 bg-black/40 p-4 text-white/82 shadow-2xl backdrop-blur-xl md:left-auto md:right-6 md:top-24 md:w-[360px] md:bottom-auto">
      <div className="flex items-start justify-between gap-3"><div><p className="text-xs uppercase tracking-[0.24em] text-white/45">{element.type}</p><h3 className="mt-1 text-lg font-medium text-white">{element.title}</h3>{element.subtitle ? <p className="mt-1 text-sm text-white/62">{element.subtitle}</p> : null}</div><button type="button" onClick={onClose} aria-label="Close Ground detail" className="rounded-full bg-white/10 px-2 py-1 text-white/70">×</button></div>
      ...
      <dl className="mt-4 grid gap-1 text-xs text-white/45"><div className="flex justify-between gap-3"><dt>Date</dt><dd>{formatDate(element.createdAt)}</dd></div><div className="flex justify-between gap-3"><dt>Layer</dt><dd>{layer}</dd></div><div className="flex justify-between gap-3"><dt>State</dt><dd>{element.state}</dd></div></dl>
      <div className="mt-4 flex flex-wrap gap-2">
        ...
        {!locked ? <button type="button" onClick={createRitual} className="rounded-full bg-white/10 px-3 py-2 text-xs text-white/84">Create grounding ritual</button> : null}
