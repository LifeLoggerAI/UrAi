...
function Toggle({ label, checked, onChange, description }: { label: string; checked: boolean; onChange: (checked: boolean) => void; description?: string }) {
  return (
    <label className="flex items-start justify-between gap-4 rounded-2xl bg-white/[0.055] p-3 text-sm text-white/78">
      <span><span className="block font-medium text-white/88">{label}</span>{description ? <span className="mt-1 block text-xs leading-5 text-white/48">{description}</span> : null}</span>
...
function Range({ label, value, onChange }: { label: string; value: number; onChange: (value: number) => void }) {
  return <label className="block rounded-2xl bg-white/[0.055] p-3 text-sm text-white/78"><span className="font-medium text-white/88">{label}</span><input aria-label={label} className="mt-3 w-full" type="range" min={0} max={1} step={0.05} value={value} onChange={(event) => onChange(Number(event.target.value))} /></label>;
}

function NumberInput({ label, value, onChange }: { label: string; value: number; onChange: (value: number) => void }) {
  return <label className="block rounded-2xl bg-white/[0.055] p-3 text-sm text-white/78"><span className="font-medium text-white/88">{label}</span><input aria-label={label} className="mt-2 w-full rounded-xl bg-black/20 px-3 py-2 text-white" type="number" min={0} value={value} onChange={(event) => onChange(Math.max(0, Number(event.target.value || 0)))} /></label>;
}
...
export function UraiSettingsControlMenu({ isOpen, onClose, initialSection, onOpenPassport, onOpenExportCenter }: UraiSettingsControlMenuProps) {
  ...
  const renderSection = () => {
    ...
    if (active === "notifications") return <div className="space-y-3"><p className="text-sm leading-6 text-white/66">URAI should whisper only when it matters.</p><Toggle label="Notifications enabled" checked={notifications.notificationsEnabled} onChange={notifications.setNotificationsEnabled} /><Toggle label="In-app whispers" checked={notifications.timingProfile.allowInApp} onChange={(checked) => notifications.updateTimingProfile({ allowInApp: checked })} /><Toggle label="Push" checked={notifications.timingProfile.allowPush} onChange={(checked) => notifications.updateTimingProfile({ allowPush: checked })} description="Browser permission is only requested after you choose push in Passport." /><Toggle label="SMS" checked={notifications.timingProfile.allowSms} onChange={(checked) => notifications.updateTimingProfile({ allowSms: checked })} /><Toggle label="Email" checked={notifications.timingProfile.allowEmail} onChange={(checked) => notifications.updateTimingProfile({ allowEmail: checked })} /><Toggle label="Quiet hours" checked={notifications.timingProfile.quietHoursEnabled} onChange={(checked) => notifications.updateTimingProfile({ quietHoursEnabled: checked })} /><div className="grid gap-3 md:grid-cols-2"><NumberInput label="Max per day" value={notifications.timingProfile.maxPerDay} onChange={(value) => notifications.updateTimingProfile({ maxPerDay: value })} /><NumberInput label="Max per week" value={notifications.timingProfile.maxPerWeek} onChange={(value) => notifications.updateTimingProfile({ maxPerWeek: value })} /></div><button onClick={onOpenPassport} className="rounded-full bg-white/[0.07] px-4 py-2 text-sm text-white/68">Open Notification Center</button></div>;
    if (active === "sensory") return <div className="space-y-3"><p className="text-sm leading-6 text-white/66">Make URAI quieter.</p><Toggle label="Reduced sensory mode" checked={settings.settingsProfile.controlState.reducedSensoryMode} onChange={(checked) => { settings.setReducedSensoryMode(checked); audio.setReducedSensoryMode(checked); notifications.updateTimingProfile({ reducedNotificationMode: checked, maxPerDay: checked ? 1 : notifications.timingProfile.maxPerDay }); }} /><Toggle label="Reduced motion" checked={settings.settingsProfile.controlState.reducedMotion} onChange={settings.setReducedMotion} /><Toggle label="Ambient animation" checked={settings.settingsProfile.controlState.ambientAnimationEnabled} onChange={settings.setAmbientAnimationEnabled} /><Toggle label="Haptics" checked={settings.settingsProfile.controlState.hapticsEnabled} onChange={(checked) => { settings.setHapticsEnabled(checked); audio.setHapticsEnabled(checked); }} /><button onClick={() => setConfirmAction("quiet")} className="rounded-full bg-white/12 px-4 py-2 text-sm text-white">Make everything quieter</button></div>;
        ...
        <motion.section role="dialog" aria-modal="true" aria-labelledby="urai-settings-title" className="fixed inset-0 z-[110] grid place-items-end bg-black/35 p-0 text-white backdrop-blur-md md:place-items-center md:p-6" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
          <motion.div className="relative flex h-[92vh] w-full max-w-6xl flex-col overflow-hidden rounded-t-[2rem] border border-white/10 bg-slate-950/86 shadow-2xl md:h-[82vh] md:rounded-[2rem]" initial={reduceMotion ? { opacity: 1 } : { opacity: 0, y: 30, scale: 0.98 }} animate={reduceMotion ? { opacity: 1 } : { opacity: 1, y: 0, scale: 1 }} exit={reduceMotion ? { opacity: 0 } : { opacity: 0, y: 18, scale: 0.98 }}>
            ...
            <header className="relative z-10 flex items-start justify-between gap-4 border-b border-white/10 p-5">
              <div><h2 id="urai-settings-title" className="text-2xl font-medium text-white">URAI Settings</h2><p className="mt-1 text-sm text-white/58">Control what opens, speaks, remembers, notifies, and leaves.</p></div>
            ...
            <div className="relative z-10 grid min-h-0 flex-1 md:grid-cols-[260px_1fr]">
              <nav className="flex gap-2 overflow-x-auto border-b border-white/10 p-4 md:block md:space-y-2 md:overflow-y-auto md:border-b-0 md:border-r" aria-label="Settings sections">
                {SETTINGS_SECTIONS.map((section) => <button key={section.id} type="button" onClick={() => settings.setActiveSection(section.id)} className={`min-w-fit rounded-2xl px-3 py-2 text-left text-sm md:block md:w-full ${active === section.id ? "bg-white/14 text-white" : "bg-white/[0.045] text-white/58"}`}><span className="block font-medium">{section.label}</span><span className="hidden text-xs text-white/38 md:block">{section.description}</span></button>)}
              </nav>
              <main className="min-h-0 overflow-y-auto p-5">
                {renderSection()}
              </main>
            </div>
            {confirmAction ? <div className="absolute inset-0 z-20 grid place-items-center bg-black/50 p-5"><div role="alertdialog" aria-modal="true" className="w-full max-w-md rounded-[1.5rem] border border-white/10 bg-slate-950 p-5 shadow-2xl"><h3 className="text-lg font-medium text-white">Confirm carefully</h3><p className="mt-2 text-sm leading-6 text-white/66">{confirmCopy}</p><div className="mt-5 flex flex-wrap gap-2"><button type="button" onClick={runConfirm} className="rounded-full bg-white/12 px-4 py-2 text-sm text-white">Confirm</button><button type="button" onClick={() => setConfirmAction(null)} className="rounded-full bg-white/[0.07] px-4 py-2 text-sm text-white/66">Cancel</button></div></div></div> : null}
          </motion.div>
        </motion.section>
      ) : null}
    </AnimatePresence>
