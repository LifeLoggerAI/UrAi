...
function makeLineGeometry(previous: LifeMapStar, star: LifeMapStar) {
  const start = new THREE.Vector3(previous.position3D.x, previous.position3D.y, previous.position3D.z);
  const end = new THREE.Vector3(star.position3D.x, star.position3D.y, star.position3D.z);
...
export default function ConstellationLines({ stars, constellations, selectedConstellationId }: ConstellationLinesProps) {
  ...
  const lineObjects = useMemo(() => {
    return constellations.flatMap((constellation) => {
      ...
      return points.slice(1).flatMap((star, index) => {
        ...
        return [
          { id: `${constellation.id}-${previous.id}-${star.id}-main`, object: main, active, baseOpacity: active ? 0.62 : 0.25 },
          { id: `${constellation.id}-${previous.id}-${star.id}-glow`, object: glow, active, baseOpacity: active ? 0.16 : 0.055 },
        ];
      });
    });
  ...
  useFrame((state) => {
    ...
    groupRef.current.children.forEach((child, index) => {
      ...
      const userData = child.userData as { baseOpacity?: number; active?: boolean };
      const baseOpacity = userData.baseOpacity ?? 0.18;
      const shimmer = Math.sin(state.clock.elapsedTime * 0.9 + index * 0.7) * (userData.active ? 0.055 : 0.025);
      material.opacity = Math.max(0.02, baseOpacity + shimmer);
    });
    groupRef.current.rotation.y = Math.sin(state.clock.elapsedTime * 0.018) * 0.018;
  });

  return (
    <group ref={groupRef}>
      {lineObjects.map((line) => {
        line.object.userData = { baseOpacity: line.baseOpacity, active: line.active };
        return <primitive key={line.id} object={line.object} />;
      })}
    </group>
  );
}
