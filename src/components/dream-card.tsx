...
export function DreamCard({ dream }: { dream: Dream }) {
  ...
  const sentiment =
    dream.sentimentScore > 0.1 ? 'positive' : dream.sentimentScore < -0.1 ? 'negative' : 'neutral';
          ...
          <div className='w-full'>
            <div className='flex items-center gap-3 text-sm'>
              ...
              <Progress
                ...
                className='w-full'
                indicatorClassName={getProgressColor(dream.sentimentScore)}
              />
              <span className='text-muted-foreground text-xs w-10 text-right'>
                {dream.sentimentScore.toFixed(2)}
              </span>
            </div>
          </div>
