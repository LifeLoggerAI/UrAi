    ...
    <div className="urai-sky-architecture" aria-hidden>
      <svg viewBox="0 0 1600 900" preserveAspectRatio="none">
        <defs>
          ...
          <filter id="skyLineBlur">
            <feGaussianBlur stdDeviation="1.4" />
          </filter>
        </defs>

        <path
          d="M120 330 C325 190 510 250 705 135 C905 15 1120 92 1472 18"
          stroke="url(#skyLineGlow)"
          strokeWidth="2"
          fill="none"
          filter="url(#skyLineBlur)"
        />
        <path
          d="M70 560 C350 378 520 508 740 328 C963 145 1112 232 1540 182"
          stroke="rgba(122,220,255,.095)"
          strokeWidth="2"
          fill="none"
        />
        <path
          d="M260 210 C520 300 705 205 910 300 C1128 402 1280 345 1510 435"
          stroke="rgba(255,232,180,.07)"
          strokeWidth="1.5"
          fill="none"
        />
...
function FinalTerrain() {
  return (
    <div className="urai-hero-terrain" aria-hidden>
      ...
      <svg
        className="urai-hero-terrain-svg"
      ...
      >
        ...
        <path
          d="M0 210 C142 182 210 214 318 172 C430 128 540 180 650 140 C794 86 898 164 1048 122 C1218 74 1342 132 1600 92 L1600 720 L0 720 Z"
          fill="url(#heroRidgeA)"
          opacity=".55"
        />
        <path
          d="M0 310 C122 265 230 292 352 245 C488 192 596 250 724 204 C858 158 982 224 1118 185 C1304 132 1428 188 1600 162 L1600 720 L0 720 Z"
          fill="url(#heroRidgeB)"
          opacity=".88"
        />
        <ellipse
          ...
          rx="575"
          ry="140"
          fill="url(#heroGroundBloom)"
          filter="url(#heroTerrainBlur)"
          opacity=".82"
        />
        <path
          d="M0 435 C160 384 282 430 436 394 C574 362 692 416 840 382 C1004 344 1132 414 1292 376 C1425 344 1510 370 1600 346 L1600 720 L0 720 Z"
          fill="url(#heroRidgeC)"
        />
        <ellipse
          cx="800"
          ...
          rx="390"
          ry="96"
          fill="rgba(68,209,242,.16)"
          filter="url(#heroTerrainBlur)"
          opacity=".78"
        />
        <path
          d="M0 620 C188 560 388 620 590 576 C790 532 930 632 1140 568 C1338 508 1462 592 1600 538 L1600 720 L0 720 Z"
          fill="rgba(0,1,5,.95)"
        />
      </svg>
      ...
      <div className="urai-foundation-glow-lines">
        <span />
        <span />
        <span />
        <span />
...
export function HomeScene() {
  ...
  const [messages, setMessages] = useState<OrbMessage[]>([
    {
      id: "urai-wake",
      role: "urai",
      text: "I am awake in the orb. Ask me what your sky is trying to show you.",
    },
  ]);
  ...
  const openSky = () => {
    if (isTransitioning || layerOpen) return;
    beginAscent();
  };
  ...
  const submitOrbMessage = (event: FormEvent<HTMLFormElement>) => {
    ...
    setMessages((current) => [
      ...
      { id: `urai-${timestamp}`, role: "urai", text: nextOrbReply(text) },
    ]);
  ...
  return (
    <main className={`urai-hero-home phase-${phase} layer-${layer}`}>
      ...
      <button
        className="urai-sky-zone"
        type="button"
        aria-label="Ascend through the sky into the Memory Galaxy"
        disabled={isTransitioning || layerOpen}
        onClick={openSky}
      >
      ...
      <button
        className="urai-ground-zone"
        type="button"
        aria-label="Enter URAI ground foundation"
        disabled={isTransitioning || layerOpen}
        onClick={(event) => {
          event.stopPropagation();
          openGround();
      ...
      <motion.div
        ...
        animate={
          phase === "portal"
            ? { scale: 8.5, y: "-3vh", opacity: 0.95 }
            : phase === "lift"
              ? { scale: 1.14, y: "-3vh", opacity: 1 }
              : layer === "orbChat"
                ? { scale: 2.15, y: "-4vh", opacity: 0.42 }
                : phase === "ignition"
                  ? { scale: 1.06, opacity: 1 }
                  : { scale: 1, y: 0, opacity: 1 }
        }
        transition={{
          duration: phase === "portal" ? 0.52 : 0.42,
          ease: cinematicEase,
        }}
        aria-hidden
      >
      ...
      {layer === "orbChat" && (
        ...
        >
          <div
            className="urai-orb-chat"
            onClick={(event) => event.stopPropagation()}
          >
            <button
              type="button"
              className="urai-layer-close"
              onClick={closeLayer}
              aria-label="Close orb companion"
            >
              Return
            </button>
            ...
            <div className="urai-chat-stream">
              {messages.map((message) => (
                <div
                  key={message.id}
                  className={`urai-chat-message ${message.role}`}
                >
                  {message.text}
                </div>
              ))}
            ...
            <form className="urai-chat-form" onSubmit={submitOrbMessage}>
              <input
                value={orbInput}
                onChange={(event) => setOrbInput(event.target.value)}
                placeholder="Ask the orb what is forming..."
      ...
      {layer === "groundZoom" && (
        <section
          className="urai-ground-layer"
          onClick={closeLayer}
          role="dialog"
          aria-modal="true"
          aria-label="URAI ground foundation layer"
        >
          <div
            className="urai-ground-sanctum"
            onClick={(event) => event.stopPropagation()}
          >
            <button
              type="button"
              className="urai-layer-close"
              onClick={closeLayer}
              aria-label="Return home from foundation"
            >
              Return
            </button>

            <div className="urai-root-field" aria-hidden>
            ...
            <span>
              Roots, body-memory, rhythm, silence, and buried signal live here.
              This is the symbolic base layer under the Memory Galaxy.
            </span>

            <div className="urai-foundation-signals">
              <span>Breath rhythm steadying</span>
              <span>Recurring evening reflection</span>
              <span>Unspoken threshold memory</span>
            </div>
          </div>
        </section>
      )}
