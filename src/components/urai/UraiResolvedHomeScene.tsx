...
const GROUND_ZONES: Array<{ id: GroundZone; x: number; y: number; size: number; aria: string }> = [
  { id: "shadow", x: 24, y: 58, size: 126, aria: "Open shadow terrain" },
  { id: "recovery", x: 50, y: 46, size: 170, aria: "Open recovery bloom terrain" },
  { id: "memory", x: 72, y: 56, size: 134, aria: "Open memory terrain" },
  { id: "signal", x: 51, y: 74, size: 108, aria: "Open signal pool" },
];
...
function deriveHomeVisuals(home: UraiHomeViewModel): HomeVisuals {
  if (home.visualState === "threshold" || home.thresholdRisk > 0.7) {
    return {
      ...
      skyMid: "#0c1326",
      skyBottom: "#130f22",
      aura: "rgba(203,184,255,0.58)",
      auraSoft: "rgba(126,34,206,0.2)",
      horizon: "rgba(245,158,11,0.22)",
      particleOpacity: 0.2,
...
export default function UraiResolvedHomeScene() {
  ...
  const pulseOrb = () => {
    ...
    setActiveNode(null);
    setOrbCharge((charge) => {
      const next = Math.min(3, charge + 1);
  ...
  const sceneStyle = {
    ...
    "--recovery-score": `${home.recoveryScore}%`,
    "--shadow-load": `${Math.round(home.shadowLoad * 100)}%`,
    "--cognitive-load": `${Math.round(home.cognitiveLoad * 100)}%`,
  } as CSSProperties;
...
const styles = `
  ...
  .lifemap-shell{background:#020617}.return-home{position:fixed;left:calc(env(safe-area-inset-left) + 1rem);top:calc(env(safe-area-inset-top) + 1rem);z-index:70;width:44px;height:44px;border:1px solid rgba(186,230,253,.22);border-radius:999px;background:radial-gradient(circle,rgba(255,255,255,.16),rgba(1,6,18,.72));color:rgba(255,255,255,.9);backdrop-filter:blur(18px);cursor:pointer}.return-home:before{content:"";position:absolute;left:17px;top:13px;width:12px;height:12px;border-left:2px solid rgba(255,255,255,.8);border-bottom:2px solid rgba(255,255,255,.8);transform:rotate(45deg)}
  ...
  .sky-gradient{position:absolute;inset:0;background:radial-gradient(ellipse at 50% 57%,var(--aura-soft),transparent 31%),radial-gradient(ellipse at 50% 82%,rgba(125,211,252,.12),transparent 31%),linear-gradient(180deg,var(--sky-top),var(--sky-mid) 48%,var(--sky-bottom));}
  .distant-aurora{position:absolute;left:22%;right:18%;top:11%;height:22vh;border-radius:50%;background:radial-gradient(ellipse at 50% 50%,rgba(125,211,252,.1),transparent 66%);filter:blur(34px);opacity:.56;animation:auroraDrift 18s ease-in-out infinite alternate}
  ...
  .horizon-glow{position:absolute;left:50%;bottom:22%;width:min(980px,82vw);height:25vh;transform:translateX(-50%);border-radius:50%;background:radial-gradient(ellipse at 50% 38%,var(--horizon),rgba(125,211,252,.1) 36%,transparent 72%);filter:blur(26px);opacity:.92}
  .sky-vignette{position:absolute;inset:0;background:radial-gradient(ellipse at 50% 52%,transparent 0%,transparent 42%,rgba(0,0,0,.2) 72%,rgba(0,0,0,var(--vignette-opacity)) 100%),linear-gradient(180deg,rgba(0,0,0,.55),transparent 31%,rgba(0,0,0,.38));pointer-events:none}
  ...
  .body-field{position:absolute;inset:0;z-index:18;pointer-events:none}.silhouette-button{position:absolute;left:calc(50% - min(9.4vw,118px));top:58%;z-index:20;width:118px;height:320px;transform:translate(-50%,-24%);border:0;background:transparent;pointer-events:auto;cursor:pointer;opacity:.54;filter:drop-shadow(0 0 34px rgba(103,232,249,.14));transition:opacity .2s ease,transform .2s ease}.silhouette-button:hover,.silhouette-button:focus-visible{opacity:.82;transform:translate(-50%,-25%)}.body-halo{position:absolute;inset:-2% -45%;border-radius:50%;background:radial-gradient(ellipse at 50% 42%,rgba(186,230,253,.14),transparent 64%);filter:blur(18px)}.body-head{position:absolute;left:50%;top:1%;width:54px;height:70px;transform:translateX(-50%);border-radius:999px;background:linear-gradient(180deg,rgba(207,250,254,.25),rgba(125,211,252,.1));box-shadow:0 0 28px rgba(125,211,252,.12)}.body-core{position:absolute;left:50%;top:20%;width:76px;height:238px;transform:translateX(-50%);border-radius:52% 52% 38% 38%;background:linear-gradient(180deg,rgba(188,235,255,.2),rgba(96,165,250,.11) 42%,rgba(2,6,23,.07) 82%,transparent);clip-path:polygon(42% 0,58% 0,82% 22%,92% 72%,70% 100%,30% 100%,8% 72%,18% 22%);box-shadow:inset 10px 0 20px rgba(255,255,255,.045),inset -14px -18px 30px rgba(2,6,23,.16)}.body-ground-glow{position:absolute;left:50%;bottom:12%;width:150px;height:80px;transform:translateX(-50%);border-radius:50%;background:radial-gradient(ellipse,rgba(125,211,252,.12),transparent 70%);filter:blur(14px)}
  .aura-orb-button{position:absolute;left:50%;top:58%;z-index:24;width:400px;height:400px;transform:translate(-50%,-38%);border:0;border-radius:999px;background:transparent;pointer-events:auto;cursor:pointer;transition:transform .24s ease;outline:0}.aura-orb-button:hover,.aura-orb-button:focus-visible{transform:translate(-50%,-39%) scale(1.025)}.aura-field{position:absolute;inset:-32%;border-radius:999px;background:radial-gradient(circle,var(--aura),var(--aura-soft) 34%,rgba(14,165,233,.06) 52%,transparent 70%);filter:blur(30px);animation:auraPulse var(--aura-pulse) ease-in-out infinite}.aura-backlight{position:absolute;inset:13%;border-radius:999px;background:radial-gradient(circle,rgba(255,255,255,.2),rgba(103,232,249,.13) 30%,transparent 68%);filter:blur(22px)}.aura-ring{position:absolute;border-radius:999px;border:1px solid rgba(255,255,255,.14);box-shadow:0 0 34px rgba(125,211,252,.13);animation:ringRotate 14s linear infinite}.aura-ring-one{inset:17%;transform:rotate(-12deg) scaleX(1.12) scaleY(.9)}.aura-ring-two{inset:8%;opacity:.42;animation-duration:20s;animation-direction:reverse;transform:rotate(22deg) scaleX(1.15) scaleY(.88)}.aura-ring-three{inset:25%;opacity:.24;animation-duration:26s;transform:rotate(54deg) scaleX(1.2) scaleY(.84)}.orb-shadow{position:absolute;left:50%;top:64%;width:154px;height:42px;transform:translateX(-50%);border-radius:50%;background:rgba(0,0,0,.46);filter:blur(8px);opacity:.76}.orb-core{position:absolute;left:50%;top:50%;width:150px;height:150px;transform:translate(-50%,-50%);border-radius:999px;background:radial-gradient(circle at 30% 24%,#ffffff 0 9%,#d9fbff 17%,#7cf4ff 35%,#1fb7c9 51%,#0e627a 68%,#061728 92%);box-shadow:inset -26px -30px 40px rgba(1,5,14,.7),inset 18px 13px 23px rgba(255,255,255,.25),0 0 44px rgba(255,255,255,.4),0 0 132px rgba(103,232,249,.48),0 0 210px rgba(59,130,246,.22)}.orb-glass{position:absolute;left:calc(50% - 31px);top:calc(50% - 48px);width:62px;height:52px;border-radius:50%;background:radial-gradient(circle,rgba(255,255,255,.9),rgba(255,255,255,.2) 52%,transparent 72%);filter:blur(3px);transform:rotate(-18deg)}.orb-reflection{position:absolute;left:50%;top:69%;width:126px;height:34px;transform:translateX(-50%);border-radius:50%;background:rgba(2,6,23,.34);filter:blur(6px)}.orb-charge{position:absolute;left:50%;top:50%;width:12px;height:12px;border-radius:999px;background:#fff;box-shadow:0 0 22px rgba(255,255,255,.8);opacity:.16;transform-origin:0 0}.charge-one{transform:rotate(20deg) translate(105px)}.charge-two{transform:rotate(142deg) translate(118px)}.charge-three{transform:rotate(262deg) translate(104px)}[data-orb-charge="1"] .charge-one,[data-orb-charge="2"] .charge-one,[data-orb-charge="2"] .charge-two,[data-orb-charge="3"] .orb-charge{opacity:1;animation:twinkle 1.6s ease-in-out infinite}.companion-dot{position:absolute;right:calc(50% - 134px);top:60%;z-index:30;width:52px;height:52px;border:1px solid rgba(255,255,255,.2);border-radius:999px;background:transparent;box-shadow:0 0 36px rgba(103,232,249,.34);pointer-events:auto;cursor:pointer;animation:companionBreath 5.6s ease-in-out infinite}.companion-dot span{position:absolute;inset:9px;border-radius:999px;background:radial-gradient(circle,#e0f2fe,rgba(103,232,249,.34),rgba(255,255,255,.04));}
  .emotional-biome{position:absolute;left:50%;bottom:-9vh;z-index:19;width:min(1280px,94vw);height:39vh;transform:translateX(-50%);pointer-events:none}.biome-atmosphere{position:absolute;left:4%;right:4%;top:-28%;height:34%;border-radius:50%;background:radial-gradient(ellipse at 50% 80%,rgba(125,211,252,.16),transparent 70%);filter:blur(20px)}.biome-terrain{position:absolute;inset:0;border-radius:55% 55% 0 0;background:radial-gradient(ellipse at 50% 6%,rgba(226,249,255,.18),rgba(125,211,252,.08) 28%,transparent 52%),linear-gradient(180deg,rgba(18,58,91,.3),rgba(4,18,38,.72) 64%,rgba(0,0,0,.95));box-shadow:0 -28px 100px rgba(125,211,252,.14),inset 0 1px 0 rgba(255,255,255,.18)}.biome-rim{position:absolute;left:5%;right:5%;top:7%;height:2px;background:linear-gradient(90deg,transparent,rgba(226,242,255,.42),rgba(125,211,252,.25),transparent);filter:blur(.3px)}.biome-horizon{position:absolute;left:12%;right:12%;top:16%;height:1px;background:linear-gradient(90deg,transparent,rgba(226,242,255,.28),transparent);filter:blur(.5px)}.biome-recovery,.biome-shadow,.biome-memory,.biome-signal{position:absolute;border-radius:999px;filter:blur(22px);opacity:.54;transition:opacity .2s ease,transform .2s ease}.biome-recovery{left:39%;top:20%;width:22%;height:30%;background:radial-gradient(ellipse,rgba(94,234,212,.32),transparent 70%)}.biome-shadow{left:16%;top:28%;width:25%;height:32%;background:radial-gradient(ellipse,rgba(168,85,247,.23),transparent 72%)}.biome-memory{right:15%;top:30%;width:25%;height:33%;background:radial-gradient(ellipse,rgba(251,191,36,.2),transparent 72%)}.biome-signal{left:43%;top:52%;width:16%;height:26%;background:radial-gradient(ellipse,rgba(125,211,252,.24),transparent 72%)}.ground-hotspot{position:absolute;z-index:3;transform:translate(-50%,-50%);border:0;border-radius:999px;background:transparent;pointer-events:auto;cursor:pointer}.ground-hotspot:after{content:"";position:absolute;inset:35%;border-radius:999px;background:rgba(255,255,255,.18);box-shadow:0 0 26px rgba(255,255,255,.24);opacity:.14;transition:opacity .2s ease,transform .2s ease}.ground-hotspot:hover:after,.ground-hotspot:focus-visible:after{opacity:.72;transform:scale(1.2)}[data-ground="recovery"] .biome-recovery,[data-ground="shadow"] .biome-shadow,[data-ground="memory"] .biome-memory,[data-ground="signal"] .biome-signal{opacity:1;transform:scale(1.18)}
  .scene-pulse{position:absolute;left:50%;bottom:calc(env(safe-area-inset-bottom) + 1.15rem);z-index:62;display:grid;grid-template-columns:42px minmax(0,1fr) 42px;align-items:center;gap:10px;width:min(360px,calc(100vw - 2rem));transform:translateX(-50%);border:1px solid rgba(186,230,253,.18);border-radius:999px;background:rgba(1,7,20,.58);padding:8px;box-shadow:0 22px 80px rgba(0,0,0,.34),inset 0 1px 0 rgba(255,255,255,.08);backdrop-filter:blur(22px)}.scene-pulse strong{overflow:hidden;color:rgba(255,255,255,.9);font-size:.78rem;text-align:center;text-overflow:ellipsis;white-space:nowrap;text-transform:capitalize}.pulse-close,.pulse-map{width:38px;height:38px;border:1px solid rgba(255,255,255,.12);border-radius:999px;background:rgba(255,255,255,.06);cursor:pointer}.pulse-close:before,.pulse-close:after{content:"";position:absolute;width:14px;height:2px;background:rgba(255,255,255,.7);transform:translate(-50%,-50%) rotate(45deg)}.pulse-close:after{transform:translate(-50%,-50%) rotate(-45deg)}.pulse-map:before{content:"";position:absolute;width:14px;height:14px;border:2px solid rgba(255,255,255,.76);border-radius:50%;transform:translate(-50%,-50%)}.pulse-map:after{content:"";position:absolute;width:6px;height:6px;border-right:2px solid rgba(255,255,255,.76);border-bottom:2px solid rgba(255,255,255,.76);transform:translate(-20%,-20%) rotate(-45deg)}
  .is-transitioning .sky-layer{transform:scale(1.12);opacity:.42;transition:transform .78s cubic-bezier(.22,1,.36,1),opacity .78s ease}.is-transitioning .silhouette-button,.is-transitioning .companion-dot,.is-transitioning .emotional-biome,.is-transitioning .scene-pulse{opacity:0;transition:opacity .32s ease}.is-transitioning .aura-orb-button{transform:translate(-50%,-190%) scale(.72);transition:transform .78s cubic-bezier(.22,1,.36,1)}.is-transitioning .constellation-layer{opacity:.78;transform:scale(2) translateY(-6vh);transition:transform .78s cubic-bezier(.22,1,.36,1),opacity .78s ease}
