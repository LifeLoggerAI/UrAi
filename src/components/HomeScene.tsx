...
export default function HomeScene() {
  ...
  return (
    <main className={`home-scene tone-${emotionalTone} ${isTransitioning ? "is-transitioning" : ""}`}>
      ...
      <style jsx>{`
        .home-scene { --tone-a: rgba(125,211,252,.14); --tone-b: rgba(196,181,253,.12); --tone-speed: 1; position: relative; min-height: 100dvh; overflow: hidden; background: #000; color: white; isolation: isolate; user-select: none; }
        ...
        .sky-base,.emotional-wash,.horizon-glow,.aura-column,.transition-vignette { position: fixed; inset: 0; pointer-events: none; }
        .sky-base { z-index: 0; background: radial-gradient(circle at 52% 24%, rgba(68,86,154,.72), transparent 28%), linear-gradient(180deg,#060913 0%,#101936 48%,#07100d 82%,#010203 100%); }
        .emotional-wash { z-index: 3; background: radial-gradient(circle at 50% 42%, rgba(125,211,252,.16), transparent 21%), radial-gradient(circle at 50% 58%, rgba(196,181,253,.1), transparent 26%), linear-gradient(180deg,rgba(0,0,0,0) 0%,rgba(4,8,18,.14) 62%,rgba(0,0,0,.62) 100%); mix-blend-mode: screen; opacity: .82; transition: opacity 900ms ease, transform 1200ms ease; }
        .horizon-glow { z-index: 7; inset: auto -8% 0 -8%; height: 36%; background: radial-gradient(ellipse at 50% 8%, rgba(126,220,170,.14), transparent 34%), radial-gradient(ellipse at 50% 80%, rgba(32,90,58,.32), transparent 48%), linear-gradient(180deg,rgba(0,0,0,0),rgba(7,22,14,.72) 58%,rgba(0,0,0,.9)); opacity: .72; }
        .aura-column { z-index: 12; inset: -8%; background: radial-gradient(ellipse at 50% 51%,rgba(255,255,255,.11),transparent 10%), linear-gradient(180deg,transparent 0%,rgba(125,211,252,.055) 45%,rgba(196,181,253,.04) 70%,transparent 100%); opacity: .18; filter: blur(6px); transition: opacity 1000ms ease, transform 1450ms cubic-bezier(.16,1,.3,1); }
        ...
        .sky-trigger { position: fixed; inset: 0; z-index: 10; border: 0; background: transparent; cursor: zoom-in; }
        .world-stage { position: relative; z-index: 20; min-height: 100dvh; width: 100%; display: grid; place-items: center; padding: 2rem; transition: opacity 900ms ease, transform 1450ms cubic-bezier(.16,1,.3,1), filter 900ms ease; }
        .demo-badge { position: fixed; left: 50%; top: 1.35rem; transform: translateX(-50%); z-index: 35; display: inline-flex; align-items: center; gap: .6rem; border: 1px solid rgba(255,255,255,.12); background: rgba(0,0,0,.22); border-radius: 999px; padding: .42rem .8rem; color: rgba(255,255,255,.72); backdrop-filter: blur(10px); pointer-events: none; }
        ...
        .hero-world { position: fixed; top: 13%; left: 50%; z-index: 28; width: min(720px, calc(100vw - 2rem)); transform: translateX(-50%); text-align: center; pointer-events: none; text-shadow: 0 2px 24px rgba(0,0,0,.7); }
        ...
        .companion-orb { position: fixed; left: 50%; top: 52%; z-index: 26; width: clamp(178px,18vw,270px); height: clamp(178px,18vw,270px); transform: translate(-50%,-50%); border: 0; border-radius: 999px; background: transparent; cursor: zoom-in; transition: transform 1450ms cubic-bezier(.16,1,.3,1), opacity 800ms ease, filter 800ms ease; }
        ...
        .orb-halo { inset: -26%; background: radial-gradient(circle,rgba(255,255,255,.14),var(--tone-a) 34%,transparent 68%); filter: blur(14px); opacity: .92; animation: orbBreathe calc(5.8s * var(--tone-speed)) ease-in-out infinite; }
        .orb-core { inset: 23%; background: radial-gradient(circle at 38% 32%,#fff,rgba(219,242,255,.95) 18%,rgba(125,211,252,.65) 48%,rgba(45,89,120,.22) 76%,transparent 100%); box-shadow: 0 0 24px rgba(255,255,255,.8),0 0 82px rgba(125,211,252,.45),0 0 170px rgba(196,181,253,.18); }
        ...
        .orb-label { position: absolute; left: 50%; bottom: -1.2rem; transform: translateX(-50%); border-radius: 999px; border: 1px solid rgba(255,255,255,.14); background: rgba(0,0,0,.34); padding: .45rem .9rem; font-size: .72rem; font-weight: 700; letter-spacing: .18em; text-transform: uppercase; color: rgba(255,255,255,.82); backdrop-filter: blur(10px); }
        .companion-whisper { position: fixed; right: 1.2rem; top: 1.2rem; z-index: 32; width: min(280px, calc(100vw - 2.4rem)); border: 1px solid rgba(255,255,255,.12); background: rgba(0,0,0,.24); border-radius: 18px; padding: .85rem; backdrop-filter: blur(12px); color: rgba(255,255,255,.76); pointer-events: none; }
        ...
        .quiet-dock { position: fixed; left: 50%; bottom: 1.35rem; z-index: 35; width: min(860px, calc(100vw - 2rem)); transform: translateX(-50%); display: grid; grid-template-columns: repeat(4,minmax(0,1fr)); gap: .4rem; border: 1px solid rgba(255,255,255,.12); border-radius: 30px; background: rgba(2,6,18,.34); padding: .45rem; backdrop-filter: blur(14px); opacity: .38; transition: opacity .28s ease, transform .28s ease; }
        .quiet-dock:hover,.quiet-dock:focus-within { opacity: .96; transform: translateX(-50%) translateY(-.15rem); }
        .quiet-dock button { border: 0; border-radius: 22px; background: transparent; color: rgba(255,255,255,.58); padding: .7rem .8rem; text-align: left; cursor: pointer; transition: background .2s ease,color .2s ease; }
        ...
        .floating-panel { position: fixed; left: 50%; bottom: 7.2rem; z-index: 38; width: min(520px, calc(100vw - 2rem)); transform: translateX(-50%); border: 1px solid rgba(255,255,255,.16); border-radius: 28px; background: rgba(5,9,22,.68); padding: 1.05rem; text-align: center; box-shadow: 0 32px 90px rgba(0,0,0,.34); backdrop-filter: blur(18px); animation: panelIn .3s cubic-bezier(.19,1,.22,1) both; }
        ...
        .primary-actions { position: fixed; left: 50%; top: calc(52% + min(17vw,255px)); z-index: 31; transform: translateX(-50%); display: flex; gap: .65rem; align-items: center; justify-content: center; }
        .primary-actions button,.primary-actions a { min-height: 2.7rem; border-radius: 999px; border: 1px solid rgba(255,255,255,.16); padding: .75rem 1rem; font-size: .88rem; font-weight: 700; text-decoration: none; backdrop-filter: blur(10px); transition: transform .2s ease,background .2s ease,opacity .2s ease; }
        ...
        .trust-strip,.tone-chip { position: fixed; z-index: 36; border: 1px solid rgba(255,255,255,.12); background: rgba(0,0,0,.22); color: rgba(255,255,255,.58); backdrop-filter: blur(10px); pointer-events: none; }
        ...
        @media (max-width: 900px) { .home-scene { overflow-y: auto; } .hero-world { top: 10%; } .hero-world h1 { font-size: clamp(2.2rem,12vw,4rem); } .companion-orb { top: 48%; width: 170px; height: 170px; } .primary-actions { top: calc(48% + 130px); flex-direction: column; } .quiet-dock { grid-template-columns: 1fr 1fr; bottom: 1rem; max-height: 7.2rem; overflow: auto; } .trust-strip { left: 1rem; right: 1rem; bottom: 8.9rem; transform: none; white-space: normal; text-align: center; } .companion-whisper { right: 1rem; left: 1rem; top: auto; bottom: 11.2rem; width: auto; opacity: .88; } .demo-badge { top: .8rem; } .floating-panel { bottom: 11rem; } }
        @media (prefers-reduced-motion: reduce) { .emotional-wash,.star-tunnel,.transition-vignette,.companion-orb,.aura-column,.orb-ring,.orb-halo { transition-duration: .01ms !important; animation: none !important; } }
      `}</style>
    </main>
  );
}
