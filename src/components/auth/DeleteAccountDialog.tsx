      ...
      {isOpen ? (
        <motion.div className="fixed inset-0 z-[140] grid place-items-center bg-black/65 p-4 backdrop-blur-xl" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
          <motion.div className="w-full max-w-lg rounded-[2rem] border border-white/10 bg-[#090912]/92 p-6 text-white shadow-2xl" initial={{ y: 18, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 18, opacity: 0 }}>
            ...
            <h2 className="mt-2 text-2xl font-medium">Remove your URAI account path?</h2>
            <p className="mt-3 text-sm leading-6 text-white/64">This will remove your URAI account data from the cloud path we control. Local data on this device can also be cleared if you choose.</p>
            <p className="mt-3 rounded-2xl bg-white/[0.055] p-3 text-xs leading-5 text-white/52">URAI will not claim third-party deletion here. If backend deletion is still processing, the account is marked pending deletion first.</p>
            ...
            <div className="mt-5 grid gap-2">
              <button disabled={!canDelete} onClick={() => void runDelete(false)} className="rounded-full bg-white/12 px-5 py-3 text-sm text-white disabled:opacity-40">Delete cloud account</button>
              <button disabled={!canDelete} onClick={() => void runDelete(true)} className="rounded-full bg-white px-5 py-3 text-sm font-medium text-black disabled:opacity-40">Delete cloud and local data</button>
