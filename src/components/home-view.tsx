...
export function HomeView() {
  ...
  useEffect(() => {
    if (user) {
      ...
      unsubscribes.push(
        onSnapshot(qPeople, snapshot => setPeople(snapshot.docs.map(doc => doc.data() as Person)))
      );

      const auraRef = doc(db, `users/${user.uid}/auraStates/current`);
      unsubscribes.push(
        onSnapshot(auraRef, doc => setAuraState(doc.exists() ? (doc.data() as AuraState) : null))
      );
      ...
      unsubscribes.push(
        onSnapshot(qDreams, snapshot => setDreams(snapshot.docs.map(d => d.data() as Dream)))
      );
      ...
      unsubscribes.push(
        onSnapshot(qGoals, snapshot => setGoals(snapshot.docs.map(doc => doc.data() as Goal)))
      );
      ...
      unsubscribes.push(
        onSnapshot(qTasks, snapshot => setTasks(snapshot.docs.map(doc => doc.data() as Task)))
      );
    ...
    switch (zone) {
      ...
      case 'torso':
        setPanelContent({
          ...
          content: (
            <TorsoView goals={goals || []} tasks={tasks || []} voiceEvents={voiceEvents || []} />
          ),
        });
        ...
        setPanelContent({
          ...
          content: (
            <p className='text-center text-muted-foreground mt-8'>
              Sky view with weather animations and constellation overlays coming soon.
            </p>
          ),
        });
        setActivePanel('sky');
    ...
    <>
      ...
      <div className='relative w-full h-screen flex flex-col items-center justify-center p-4 overflow-hidden text-center'>
        ...
        <div className='absolute bottom-8 text-center z-10 text-xs text-muted-foreground w-full max-w-md'>
          <p>
            Tap your avatar to explore your inner world. The sky reflects your mood. The blooms are
            moments of recovery.
          </p>
        </div>
      </div>
            ...
            <>
              ...
              {activePanel === 'settings' ? (
                ...
                panelContent?.content && (
                  <div className='py-4 my-2 text-sm rounded-md'>{panelContent.content}</div>
                )
              )}
