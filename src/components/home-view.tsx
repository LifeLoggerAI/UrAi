...
export function HomeView() {
  ...
  const [auraState, setAuraState] = useState<AuraState | null | undefined>(
    undefined
  );
  const [memoryBlooms, setMemoryBlooms] = useState<MemoryBloom[] | undefined>(
    undefined
  );
  const [dreams, setDreams] = useState<Dream[] | undefined>(undefined);
  const [voiceEvents, setVoiceEvents] = useState<VoiceEvent[] | undefined>(
    undefined
  );
  const [innerTexts, setInnerTexts] = useState<
    InnerVoiceReflection[] | undefined
  >(undefined);
  ...
  const overallMood =
    voiceEvents?.[0]?.sentimentScore ?? dreams?.[0]?.sentimentScore ?? 0;
  ...
  useEffect(() => {
    if (user) {
      ...
      unsubscribes.push(
        onSnapshot(auraRef, doc =>
          setAuraState(doc.exists() ? (doc.data() as AuraState) : null)
        )
      );

      const qBlooms = query(
        collection(db, 'users', user.uid, 'memoryBlooms'),
        orderBy('triggeredAt', 'desc'),
        limit(10)
      );
      unsubscribes.push(
        onSnapshot(qBlooms, snapshot =>
          setMemoryBlooms(
            snapshot.docs
              .map(doc => doc.data() as MemoryBloom)
      ...
      unsubscribes.push(
        onSnapshot(qInner, snapshot =>
          setInnerTexts(
            snapshot.docs.map(d => d.data() as InnerVoiceReflection)
          )
        )
      );
  ...
  const handleZoneClick = async (
    zone: 'head' | 'torso' | 'legs' | 'arms' | 'aura' | 'sky' | 'ground'
  ) => {
    ...
    switch (zone) {
      case 'head':
        setPanelContent({
          ...
          description:
            'A symbolic control room for introspection and analysis.',
          content: (
            <CognitiveZoneView
              dreams={dreams || []}
      ...
      case 'torso':
        setPanelContent({
          ...
          description:
            'Explore your inner drive, rhythms, and somatic memories.',
          content: (
            <TorsoView
              goals={goals || []}
              tasks={tasks || []}
      ...
      case 'legs':
        setPanelContent({
          ...
          description:
            'Explore your physical path, stability, and forward momentum.',
          content: <LegsView />,
        });
        setActivePanel('legs');
        return;
      case 'arms':
        setPanelContent({
          ...
          description:
            'Explore your patterns of action, effort, and social connection.',
          content: <ArmsView />,
        });
        setActivePanel('arms');
        return;
      case 'sky':
        setPanelContent({
          title: 'Sky View',
          description:
            'Explore the weather, time of day, and celestial patterns influencing your mood.',
          content: (
            <p className="text-center text-muted-foreground mt-8">
              Sky view with weather animations and constellation overlays coming
              soon.
            </p>
          ),
        });
      ...
      case 'ground':
        setPanelContent({
          title: 'Ground View',
          description:
            'Your emotional garden: soil health, recovery events, and roots of memory.',
          content: <GroundView />,
        });
      ...
      case 'aura':
        ...
        try {
          const result = await suggestRitualAction({
            zone,
            context: `Overall mood is ${moodDescription()}.`,
          });
  ...
  const handleBloomClick = (bloom: MemoryBloom) => {
    setPanelContent({
      ...
      content: (
        <p className="text-lg" style={{ color: bloom.bloomColor }}>
          {bloom.description}
        </p>
      ),
    });
  ...
  const getPanelSize = () => {
    switch (activePanel) {
      ...
      case 'ground':
        return 'max-w-4xl';
      case 'companion':
        return 'max-w-2xl h-[80vh] flex flex-col';
      default:
        return 'max-w-lg';
    }
  };
    ...
    <>
      ...
      <div className="relative w-full h-screen flex flex-col items-center justify-center p-4 overflow-hidden text-center">
              ...
              memoryBlooms.map((bloom, i) => (
                <Tooltip key={bloom.bloomId}>
                  <TooltipTrigger asChild>
                    ...
                    >
                      <Sprout
                        className="h-5 w-5 hover:scale-125 transition-transform"
                        style={{ color: bloom.bloomColor }}
                      />
                    </button>
                  </TooltipTrigger>
                  <TooltipContent>
        ...
        <div className="absolute bottom-8 text-center z-10 text-xs text-muted-foreground w-full max-w-md">
          <p>
            Tap your avatar to explore your inner world. The sky reflects your
            mood. The blooms are moments of recovery.
          </p>
        </div>
      </div>
        ...
        <AlertDialogContent className={getPanelSize()}>
          {activePanel === 'companion' ? (
            ...
            <>
              ...
              <AlertDialogFooter>
                {activePanel !== 'settings' && (
                  <AlertDialogAction onClick={() => setActivePanel(null)}>
                    Done
                  </AlertDialogAction>
                )}
                {activePanel === 'settings' && (
                  <AlertDialogCancel onClick={() => setActivePanel(null)}>
                    Close
                  </AlertDialogCancel>
                )}
              </AlertDialogFooter>
