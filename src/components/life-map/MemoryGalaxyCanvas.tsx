...
export default function MemoryGalaxyCanvas(props: MemoryGalaxyCanvasProps) {
  ...
  const selectedStar = visibleStars.find((star) => star.id === props.selectedStarId) ?? lifeMapMockData.memoryStars.find((star) => star.id === props.selectedStarId);

  useEffect(() => {
    if (props.cameraCommand === "idle") return;
    const delay = props.reducedMotion ? 80 : props.cameraCommand === "replay" ? 1100 : props.cameraCommand === "focus" ? 900 : 700;
    const timer = window.setTimeout(props.onCameraCommandComplete, delay);
    return () => window.clearTimeout(timer);
  }, [props.cameraCommand, props.onCameraCommandComplete, props.reducedMotion]);

  const stageTransform = props.reducedMotion
    ? "translate3d(0,0,0) scale(1)"
    : props.cameraCommand === "focus"
      ? "translate3d(0,-1.5%,0) scale(1.08) rotate(-1deg)"
      : props.cameraCommand === "replay"
        ? "translate3d(0,-3%,0) scale(1.12) rotate(-2deg)"
        : props.cameraCommand === "mirror"
    ...
    <div className="absolute inset-0 overflow-hidden bg-[radial-gradient(circle_at_48%_42%,rgba(14,165,233,0.18),transparent_30%),radial-gradient(circle_at_68%_22%,rgba(168,85,247,0.14),transparent_26%),linear-gradient(180deg,#020617,#030712_62%,#07111f)]">
      ...
      <div className="absolute inset-0 transition-transform duration-[900ms] ease-out" style={{ transform: stageTransform }}>
        ...
        {visibleStars.map((star, index) => {
          ...
          const filtered = props.activeFilter !== "All" && !selected;
          ...
          return (
            <div key={star.id} className="absolute z-10 -translate-x-1/2 -translate-y-1/2" style={position}>
              ...
              >
                <span
                  className={`block rounded-full transition duration-300 ${selected ? "h-7 w-7" : "h-3 w-3 group-hover:h-4 group-hover:w-4"} ${blurred ? "blur-sm" : ""} ${props.highContrast ? "ring-2 ring-white" : ""}`}
