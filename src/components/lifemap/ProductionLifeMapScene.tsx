...
import { buildReplayBeats, interpolateReplayBeat, type ReplayBeat } from './cinematicLifeMapSystem';
...
type LifeMapProps = { forceDemo?: boolean; onRequestHome?: () => void };
type ShellStyle = CSSProperties & { '--px': string; '--py': string; '--map-x': string; '--map-y': string; '--map-scale': string; '--focus-x': string; '--focus-y': string; '--focus-scale': string };
...
type DragState = { pointerId: number; startX: number; startY: number; originX: number; originY: number; moved: boolean } | null;
...
const deepStars = Array.from({ length: 340 }, (_, index) => {
  const layer = index % 8;
  return { id: `deep-${index}`, x: (index * 37 + 11) % 100, y: (index * 61 + 7) % 100, size: layer < 2 ? 1.9 : layer < 4 ? 1.05 : 0.56, alpha: layer < 2 ? 0.78 : layer < 4 ? 0.46 : 0.22, depth: layer * 34, twinkle: 3.2 + (index % 9) * 0.43 };
});
...
function msLabel(ms: number) { const total = Math.max(0, Math.round(ms / 1000)); return `${Math.floor(total / 60)}:${String(total % 60).padStart(2, '0')}`; }
...
export default function ProductionLifeMapScene({ forceDemo = false, onRequestHome }: LifeMapProps) {
  ...
  const focusStar = activeReplayStar ?? selectedStar;

  const focus = focusStar ? { x: (50 - focusStar.position.x) * 0.75, y: (50 - focusStar.position.y) * 0.42, scale: panel === 'replay' ? 1.75 : 1.55 } : { x: 0, y: 0, scale: 1 };
  const shellStyle: ShellStyle = { '--px': `${pointer.x}`, '--py': `${pointer.y}`, '--map-x': `${mapOffset.x}px`, '--map-y': `${mapOffset.y}px`, '--map-scale': `${mapScale}`, '--focus-x': `${focus.x}vw`, '--focus-y': `${focus.y}vh`, '--focus-scale': `${focus.scale}` };
  const companionLine = activeReplayBeat?.narratorLine ?? selectedStar?.narratorLine ?? selectedConstellation?.narratorLine ?? 'Drag, zoom, then choose a Memory Star. URAI will open the memory, thread, and reason it appeared.';

  useEffect(() => {
    if (!replayPlaying || panel !== 'replay') return undefined;
  ...
  }, [panel, replayElapsed, replayPlaying, replayState.totalMs]);
  ...
  useEffect(() => {
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      if (panel === 'replay') returnToFocus();
      else if (panel === 'focus' || panel === 'ritual' || panel === 'privacy') returnToMap();
      else if (panel === 'scroll' || panel === 'mirror') setPanel('none');
      else onRequestHome?.();
    };
    window.addEventListener('keydown', onKeyDown);
    return () => window.removeEventListener('keydown', onKeyDown);
  }, [onRequestHome, panel, returnToFocus, returnToMap]);
  ...
  const handlePointerMove = useCallback((event: PointerEvent<HTMLElement>) => {
    ...
    if (Math.hypot(dx, dy) > 6) dragRef.current = { ...drag, moved: true };
    setMapOffset({ x: drag.originX + dx, y: drag.originY + dy });
  }, []);

  const handlePointerDown = useCallback((event: PointerEvent<HTMLElement>) => {
    if ((event.target as HTMLElement).closest('button,input,a,.bloom-sheet,.side-panel,.replay-panel')) return;
    dragRef.current = { pointerId: event.pointerId, startX: event.clientX, startY: event.clientY, originX: mapOffset.x, originY: mapOffset.y, moved: false };
  ...
  const handleWheel = useCallback((event: WheelEvent<HTMLElement>) => {
    event.preventDefault();
    setMapScale((scale) => clamp(Number((scale - event.deltaY * 0.0012).toFixed(3)), 0.62, 2.6));
  }, []);

  const handleStarPointerUp = useCallback((event: PointerEvent<HTMLButtonElement>, star: MemoryStar) => {
  ...
  return (
    <main ref={stageRef} className={cx('urai-map', panel !== 'none' && 'has-panel', panel === 'replay' && 'is-replay')} style={shellStyle} onPointerDown={handlePointerDown} onPointerMove={handlePointerMove} onPointerUp={handlePointerUp} onPointerCancel={handlePointerUp} onWheel={handleWheel} aria-label="URAI Symbolic Life Map">
      ...
      <header className="map-header">
        ...
        <span>{stars.length} Memory Stars · drag to pan · wheel/pinch to zoom · Esc unwinds</span>
      </header>

      <button type="button" className="home-link" onClick={onRequestHome}>Home</button>
      <aside className="companion" aria-live="polite"><strong>Companion</strong><span>{companionLine}</span><small>{panel === 'none' ? 'Map is open.' : panel === 'replay' ? 'Replay is moving through the thread.' : 'Focus is held.'}</small></aside>
      ...
      <section className="camera-stage">
        <div className="map-transform">
          ...
          {stars.map((star) => {
            ...
            const style: StarStyle = { '--x': `${star.position.x}%`, '--y': `${star.position.y}%`, '--z': `${star.position.z}px`, '--size': `${Math.max(12, star.visual.size * 0.86)}px`, '--hue': toneHue(star.emotionalTone), '--aura': `${Math.max(30, star.visual.auraRadius * 0.8)}px`, '--pulse': `${1.8 + star.visual.pulseSpeed * 2.8}s`, '--alpha': `${Math.max(0.55, star.confidence)}` };
            return <button key={star.id} type="button" className={starClass(star, isActive, isRelated, star.id === activeReplayBeat?.starId)} style={style} onPointerUp={(event) => handleStarPointerUp(event, star)} aria-label={`Focus Memory Star: ${star.title}`}><span className="core" /><span className="ray ray-x" /><span className="ray ray-y" /><span className="label"><b>{star.title}</b><small>{star.subtitle}</small></span></button>;
          })}
        </div>
      </section>
