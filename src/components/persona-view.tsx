            ...
            <ul className='space-y-2'>
              {traitChanges.map((change, index) => (
                <li key={index} className='text-sm'>
                  <span className='font-semibold capitalize'>{change.trait}</span> changed from{' '}
                  {change.from} to {change.to} on {new Date(change.date).toLocaleDateString()}.
                </li>
              ))}
            </ul>
          </CardContent>
      ...
      <div className='grid md:grid-cols-2 gap-4'>
          ...
          <Card>
            <CardHeader>
              <CardTitle className='flex items-center gap-2 text-base'>
                <Lightbulb className='h-5 w-5 text-green-500' /> Peak Productivity
              </CardTitle>
            </CardHeader>
        ...
        {conflictEvents && (
          <Card className='md:col-span-2'>
            <CardHeader>
              <CardTitle className='flex items-center gap-2 text-base'>
                <ShieldAlert className='h-5 w-5 text-red-500' /> Conflict Triggers
              </CardTitle>
            </CardHeader>
            <CardContent className='flex flex-wrap gap-2'>
