...
export function MirrorReflectionDetail({ reflection, onClose, onOpenGround, onOpenLifeMap, onOpenPassport, onTalkToCompanion }: MirrorReflectionDetailProps) {
  ...
  const createRitual = () => {
    if (reflection.permissionRequired) return;
    const [ritual] = rituals.suggestForContext({ selectedMirrorReflection: { id: reflection.id, patternType: reflection.patternType, sourceLayerIds: layers } });
    if (ritual) rituals.startRitual(ritual.id);
  };
  const exportCard = () => {
    if (reflection.permissionRequired) return;
    exports.openExport({ id: `export-mirror-${reflection.id}`, type: "mirror_reflection_card", title: reflection.title, summary: reflection.summary, format: "png", privacyLevel: "summary_only", sourceType: "mirror", sourceIds: [reflection.id], sourceLayerIds: layers, userApproved: true });
  };
  const runAction = () => { if (reflection.suggestedAction === "open_ground") onOpenGround?.(); if (reflection.suggestedAction === "open_life_map") onOpenLifeMap?.(); if (reflection.suggestedAction === "open_passport") onOpenPassport?.(); if (reflection.suggestedAction === "talk_to_companion") onTalkToCompanion?.(); };
  return (
    <aside className="pointer-events-auto absolute inset-x-4 bottom-4 z-30 rounded-3xl border border-white/10 bg-black/42 p-4 text-white/82 shadow-2xl backdrop-blur-xl md:left-auto md:right-6 md:top-24 md:w-[380px] md:bottom-auto">
      <div className="flex items-start justify-between gap-3"><div><p className="text-xs uppercase tracking-[0.24em] text-white/45">{reflection.patternType.replace(/_/g, " ")}</p><h3 className="mt-1 text-lg font-medium text-white">{reflection.title}</h3></div><button type="button" onClick={onClose} aria-label="Close reflection detail" className="rounded-full bg-white/10 px-2 py-1 text-white/70">×</button></div>
      <p className="mt-4 text-sm leading-6 text-white/72">{reflection.summary}</p>
      <div className="mt-4 rounded-2xl bg-white/[0.055] p-3 text-sm leading-6 text-white/64"><p>{whyText(reflection)}</p><p className="mt-2">{confidenceCopy[reflection.confidence]}</p></div>
      <div className="mt-4 flex flex-wrap gap-2">
        ...
        {!reflection.permissionRequired ? <button type="button" onClick={createRitual} className="rounded-full bg-white/10 px-3 py-2 text-xs text-white/84">Create reflection ritual</button> : null}
        ...
        {!reflection.permissionRequired ? <button type="button" onClick={exportCard} className="rounded-full bg-white/[0.08] px-3 py-2 text-xs text-white/62">Export card</button> : null}
