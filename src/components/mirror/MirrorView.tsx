...
import { MirrorPhysicalWorld } from "./MirrorPhysicalWorld";
...
const tintByMood: Record<GenesisMoodState, string> = {
  calm: "from-slate-950/20 via-blue-950/18 to-black/45",
  heavy: "from-neutral-950/28 via-slate-950/22 to-black/55",
  focused: "from-slate-950/18 via-cyan-950/16 to-black/42",
  anxious: "from-slate-950/22 via-indigo-950/22 to-black/48",
  hopeful: "from-sky-950/12 via-sky-900/10 to-black/35",
  recovering: "from-teal-950/16 via-teal-950/12 to-black/38",
  shadow: "from-indigo-950/28 via-slate-950/24 to-black/55",
  threshold: "from-slate-950/25 via-violet-950/22 to-black/5",
  luminous: "from-slate-950/14 via-blue-900/10 to-black/34",
};
...
export function MirrorView({ isOpen, onClose, moodState = "luminous", onOpenGround, onOpenLifeMap, onOpenPassport, onTalkToCompanion }: MirrorViewProps) {
  ...
  return (
    <AnimatePresence>
      {isOpen ? (
        <motion.section role="dialog" aria-modal="true" aria-label="URAI Mirror" className="fixed inset-0 z-[70] overflow-hidden bg-[#75838b] text-white" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
          <MirrorPhysicalWorld />
          <div className={`absolute inset-0 bg-gradient-to-b ${tintByMood[moodState]}`} />
          <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_28%,rgba(210,229,235,0.09),transparent_34%),linear-gradient(180deg,rgba(8,12,26,0.08)_0%,rgba(3,5,13,0.35)_100%)]" />
          <motion.div className="absolute inset-0 opacity-35" animate={reduceMotion ? undefined : { opacity: [0.26, 0.38, 0.26] }} transition={{ duration: 16, repeat: Infinity, ease: "easeInOut" }}>
            <div className="absolute left-1/2 top-[38%] h-[34vh] w-[68vw] -translate-x-1/2 rounded-[50%] border border-white/6 bg-white/[0.012]" />
          </motion.div>

          <header className="pointer-events-auto absolute inset-x-0 top-0 z-30 flex items-start justify-between gap-4 p-4 md:p-6">
            <div>
              <p className="text-xs uppercase tracking-[0.32em] text-white/55">URAI Mirror</p>
              <h2 className="mt-1 text-xl font-medium text-white md:text-2xl">Here’s the pattern, not the judgment.</h2>
              <p className="mt-1 max-w-xl text-sm text-white/68">Reflections stay gentle, permission-aware, and uncertain when the signal is early.</p>
            </div>
            <button type="button" onClick={onClose} aria-label="Close Mirror" className="rounded-full bg-black/30 px-4 py-2 text-sm text-white/82 backdrop-blur-md">Close</button>
          </header>
          ...
          <div className="pointer-events-auto absolute inset-x-4 top-[6.25rem] z-30 flex gap-2 overflow-x-auto md:left-6 md:right-auto md:top-auto md:bottom-6">
            {availableFilters.map((item) => <button key={item.type} type="button" onClick={() => mirror.setFilterByPatternType(item.type)} className={`rounded-full px-3 py-2 text-xs backdrop-blur-md ${mirror.filterByPatternType === item.type ? "bg-white/18 text-white" : "bg-black/24 text-white/70"}`}>{item.label}</button>)}
          </div>
