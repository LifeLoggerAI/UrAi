    ...
    <div className={`urai-scene-root relative min-h-screen w-full overflow-hidden bg-[radial-gradient(circle_at_50%_32%,#203963_0%,#101a38_48%,#050714_100%)] ${className}`} data-mood={moodState}>
      <style jsx>{`
        .urai-scene-root {
          ...
          --orb-y: 58%;
          --orb-size: 190px;
          --portal-opacity: ${mood.portalOpacity};
          --vignette-opacity: ${mood.vignetteOpacity};
          --aura-opacity: ${mood.auraOpacity};
          --cloud-depth-opacity: ${mood.cloudDepthOpacity};
          --mood-overlay-opacity: ${mood.atmosphereOpacity};
        ...
        @media (prefers-reduced-motion: reduce) {
          .urai-scene-root {
            --portal-opacity: 0.28;
            --vignette-opacity: 0.36;
            --aura-opacity: 0.44;
            --cloud-depth-opacity: 0.32;
            --mood-overlay-opacity: 0.18;
          }
        }
      `}</style>
      ...
      <button type="button" aria-label="Open Settings" className="absolute bottom-[var(--passport-offset)] left-[var(--passport-offset)] z-[60] h-10 w-10 rounded-full bg-transparent focus:outline-none focus-visible:ring-2 focus-visible:ring-white/50" />
    </div>
  );
}
