...
export function OrbLayer({ intensity = 0.86, interactive = true, onClick, className = "" }: OrbLayerProps) {
  ...
  return (
    ...
    >
      <SafeLayerImage
        ...
        style={{ filter: `brightness(${0.86 + intensity * 0.42})` }}
      />
      <SafeLayerImage
        ...
        style={{ filter: `brightness(${0.9 + intensity * 0.24}) saturate(${1 + intensity * 0.18})` }}
      />
      <motion.span
        className="pointer-events-none absolute inset-[-12%] rounded-full border border-white/15"
        style={{ boxShadow: `0 0 ${42 + intensity * 54}px rgba(248,217,139,${0.18 + intensity * 0.26})` }}
        animate={reduceMotion ? undefined : { opacity: [0.42, 0.74, 0.42], rotate: 360 }}
        transition={{ opacity: { duration: 5, repeat: Infinity, ease: "easeInOut" }, rotate: { duration: 30, repeat: Infinity, ease: "linear" } }}
      />
      <SafeLayerImage
