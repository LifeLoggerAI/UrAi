...
export function TorsoView({ goals, tasks, voiceEvents }: TorsoViewProps) {
      ...
      <div>
        ...
        {goals.length > 0 ? (
          goals.map(goal => (
            <div
              key={goal.id}
              className="text-foreground/90 p-4 bg-muted rounded-lg border"
            >
              <p>{goal.title}</p>
            </div>
          ))
        ...
        {tasks.length > 0 ? (
          <ul className="space-y-3">
            {tasks.map(task => (
              <li
                key={task.id}
                className="flex items-start gap-3 p-3 bg-muted rounded-lg border"
              >
  ...
  const panels = [
    ...
    },
    {
      ...
      content: (
        <p className="text-center text-muted-foreground mt-8">
          Body Memory insights coming soon.
        </p>
      ),
    },
  ];
